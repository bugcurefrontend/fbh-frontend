...
export default function CountryAutocomplete({
    ...
    React.useEffect(() => {
        if (defaultValue) {
            const defaultId = typeof defaultValue === "number" ? defaultValue : defaultValue.id;
            ...
            .then((res) => {
                if (res?.length && Array.isArray(res)) {
                    const result: Country[] = res.map(
                        (i: { name: string; id: number; code: string; active: boolean }) => ({
                            id: i.id,
                            name: i.name,
                            code: i.code,
                            active: i.active,
            ...
            <Popover open={open} onOpenChange={handleOpenChange}>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        className={cn(
                            "font-base-size placeholder-styles h-[44px] w-full justify-between rounded-[8px] bg-white px-3 py-2 !text-base !text-[#4C4748] placeholder:text-gray-400",
                    ...
                    >
                        <span className={!value?.name ? "text-gray-400" : "text-[#090C0F]"}>
                ...
                >
                    <Command>
                        ...
                        <CommandList className="h-[150px]">
                            <CommandEmpty className="text-center py-2 text-gray-500">No countries found.</CommandEmpty>
                            <CommandGroup>
