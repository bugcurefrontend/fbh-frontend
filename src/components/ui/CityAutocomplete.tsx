...
export default function CityAutocomplete({
  ...
  const clearSearch = (e: React.MouseEvent) => {
    ...
    setSearchTerm("");
    setCities([]);
    if (onChange) onChange({ id: "", name: "" } as City);
  };
          ...
          fetch(`${API_URL}/cities/id/${value.id}.json`)
            ...
            .then((res) => {
              if (res && res.id === value.id && onChange) {
                if (res.complete_name !== value.complete_name) onChange(res);
              }
            })
            .catch(() => {
  ...
  }, [value?.id, value?.name, value?.state, value?.complete_name, API_URL]);
    ...
    async () => {
      try {
        if (searchTerm && searchTerm.length > 2 && API_URL) {
          ...
          if (!response.ok) {
            logger.error("Failed to fetch cities", { status: response.status });
            setCities([]);
            return;
          }
  ...
  const filteredCities = React.useMemo(() => {
    if (!searchTerm || searchTerm.length === 0) return cities;
