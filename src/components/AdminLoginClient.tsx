...
export default function AdminLoginClient() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Authenticate directly with Strapi and store admin auth in localStorage.
  ...
  return (
    <div className={`min-h-screen bg-white flex items-center justify-center p-4 md:p-8 ${publicSans.className}`}>
      <div className="relative w-full max-w-7xl mx-auto md:grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        <div className="relative h-[730px] w-[638px] rounded-[16px] overflow-hidden hidden md:block">
          <Image src={loginImage} alt="Login Image" fill className="object-cover" priority />
        </div>
        ...
        <div className="flex flex-col items-center justify-center w-full max-w-[375px] mx-auto space-y-8">
          ...
          <h1 className={`${playfair.className} text-2xl font-semibold text-[#232D26]`}>Admin Portal</h1>

          <form onSubmit={handleLogin} className="w-full space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#454950] block">Email Address</label>
              <div className="relative">
                <input
                  ...
                  required
                  disabled={isLoading}
                  className={`w-full px-4 py-3 rounded-[8px] border ${error ? "border-[#F04438]" : "border-[#D0D5DD]"} focus:outline-none focus:ring-none placeholder:text-[#98A2B3] text-sm transition-all placeholder:bg-white disabled:opacity-50 disabled:cursor-not-allowed`}
                />
                {error && (
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#454950] block">Password</label>
              <div className="relative">
                <input
                  type="password"
                  value={password}
                  ...
                  required
                  disabled={isLoading}
                  className={`w-full px-4 py-3 rounded-[8px] border ${error ? "border-[#F04438]" : "border-[#D0D5DD]"} focus:outline-none focus:ring-none placeholder:text-[#98A2B3] text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed`}
                />
                {error && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    <AlertCircle className="w-5 h-5 text-[#F04438]" />
            ...
            <div className="pt-3">
              ...
              {error && (
                <p className="text-[#F04438] text-xs leading-4.5 mt-1.5 text-center font-medium">{error}</p>
              )}
            </div>
          </form>
